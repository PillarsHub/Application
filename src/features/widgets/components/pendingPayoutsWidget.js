...
const PendingPayoutsWidget = ({ date, customerId }) => {
  ...
  const totalAmount = useMemo(
    () => groups.reduce((t, g) => t + (g.amount ?? 0), 0),
    [groups]
  );
        ...
        <div className="table-responsive">
          <table className="table card-table table-vcenter text-nowrap">
                                  ...
                                  <table className="table table-sm mb-0">
                                    ...
                                    <tbody>
                                      {cache.items.map((child, idx) => {
                                        ...
                                        return (
                                          <tr key={rowKey}>
                                            <td>{child.bonusTitle ?? child.bonusId ?? "Bonus"}</td>
                                            <td className="d-none d-sm-table-cell">{child.level ?? "—"}</td>
            ...
            <tfoot>
              ...
              <tr>
                <td colSpan={6} className="strong">Total Due</td>
