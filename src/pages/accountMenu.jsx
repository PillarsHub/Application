...
import { GetUser, GetScope, useToken22 } from "../features/authentication/hooks/useToken.jsx"
...
const AccountMenu = () => {
  ...
  const accountUrl = getThemeUrl(theme, 'accountUrl');
  const logoutUrl = getThemeUrl(theme, 'logoutUrl');

  var profileLink = '/account/profile';
  if (customerId != undefined) profileLink = `/customers/${customerId}/account/profile`;
  if (accountUrl) profileLink = accountUrl;

  const showProfile = !user?.SSO || accountUrl;
  const showManageAccount = !user?.SSO && customerId == undefined;
  const showLogout = !user?.SSO || logoutUrl;
  const showDropdown = showProfile || showManageAccount || showLogout;
  ...
  return <>
    ...
    {showDropdown && <>
      ...
      <div className="dropdown-menu dropdown-menu-end dropdown-menu-arrow">
        {showProfile && <a href={profileLink} className="dropdown-item" title="Manage">
        ...
        {showManageAccount && <a className="dropdown-item" href="/account" title="Manage Account">
        ...
        {(showProfile || showManageAccount) && showLogout && <div className="dropdown-divider"></div>}
        {showLogout && <button className="dropdown-item" onClick={handleLogout}>
...
function getThemeUrl(theme, urlName) {
  if (!theme || theme.init) return null;

  const domainUrl = getDomainThemeUrl(theme, urlName);
  if (domainUrl) return domainUrl;

  return theme?.[urlName] || null;
}

function getDomainThemeUrl(theme, urlName) {
  if (!Array.isArray(theme?.domains)) return null;

  const currentHost = window.location.hostname.toLowerCase();
  const match = theme.domains.find((domain) => {
    return domain?.domain?.toLowerCase() === currentHost && domain?.[urlName];
  });
